...
/* ------------------------------------------------------------
   Pull-to-refresh — only when the HUD panel is scrolled to top
   and the user drags down from within it. The 3D canvas keeps
   its own touch handling (orbit rotate) so the map isn't
   affected.
   ------------------------------------------------------------ */
(() => {
  const ARMED_AT = 90;   // px pulled → will refresh on release
  const MAX = 140;
  const el = $('#pull');
  const label = $('#pullLabel');
  let start = null;

  function panelUnder(t) {
    const p = document.querySelector('.hud-panel');
    if (!p) return null;
    const r = p.getBoundingClientRect();
    if (t.clientX < r.left || t.clientX > r.right) return null;
    if (t.clientY < r.top || t.clientY > r.bottom) return null;
    if (p.scrollTop > 0) return null;
    return p;
  }

  document.addEventListener('touchstart', (e) => {
    if (e.touches.length !== 1) return;
    const p = panelUnder(e.touches[0]);
    if (!p) return;
    start = { y: e.touches[0].clientY, panel: p };
  }, { passive: true });

  document.addEventListener('touchmove', (e) => {
    if (!start) return;
    // If the panel has already scrolled, cancel the pull.
    if (start.panel.scrollTop > 0) { cancel(); return; }
    const dy = e.touches[0].clientY - start.y;
    if (dy <= 0) { hide(); return; }
    const eased = Math.min(MAX, dy * 0.75);
    show(eased);
  }, { passive: true });
  ...
  function show(dy) {
    el.hidden = false;
    el.style.transform = `translate(-50%, ${dy - 32}px)`;
    el.dataset.dy = String(dy);
    if (dy >= ARMED_AT) { el.classList.add('armed'); label.textContent = 'Loslassen zum Neu-Laden'; }
    else                { el.classList.remove('armed'); label.textContent = 'Ziehen zum Neu-Laden'; }
  }
  function hide() {
    el.hidden = true;
    el.classList.remove('armed', 'loading');
    el.style.transform = '';
    el.dataset.dy = '0';
  }
  function cancel() { start = null; hide(); }
})();
